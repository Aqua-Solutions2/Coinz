...
class TicTacToe(Cog):
    ...
    def create_embed(self, ctx, desc, board, turn=None, winner=None, color=Color.blue()):
        ...
        embed.add_field(name="Board", value=self.create_board(board), inline=False)
        if turn is not None:
    ...
    async def tictactoe(self, ctx, member: Member, bet: int):
        ...
        message = await ctx.send(content=f"{member.mention}",
                                 embed=self.create_embed(ctx, "%s has challenged %s in a game of %s.\n%s, if you type `%s` in chat the game will start.\nYou have 60 seconds to decide." % (ctx.author.name, member.name, "Tic Tac Toe", member.name, token), board, turn.mention))
        ...
        await message.edit(embed=self.create_embed(ctx, "Choose a number where you want to place your indicator. (Number between 1 and %s.)" % int(self.BOARD_WIDTH ** 2), board, turn.mention))
        ...
        while True:
            try:
                ...
                try:
                    ...
                    await ctx.send("You have to give a valid number between %s and %s." % (1, int(self.BOARD_WIDTH ** 2)))
                    continue
                ...
                if board[index1][index2] == self.EMPTY_EMOTE:
                    ...
                    await ctx.send("This square is not empty. Choose another square.")
                ...
                if all([True if self.EMPTY_EMOTE not in item else False for item in board]):
                    ...
                    await message.edit(embed=self.create_embed(ctx, "Tie. You got your bet back.", board, color=Color.red()))
                    break

                winner = self.get_winner(board)
                if winner == self.X_EMOTE:
                    general.add_money(ctx.guild.id, ctx.author.id, bet * 2)
                    await message.edit(embed=self.create_embed(ctx, "%s has won %s%s." % (ctx.author.mention, currency, bet * 2), board, winner=ctx.author.mention, color=Color.green()))
                    break
                elif winner == self.O_EMOTE:
                    general.add_money(ctx.guild.id, member.id, bet * 2)
                    await message.edit(embed=self.create_embed(ctx, "%s has won %s%s." % (member.mention, currency, bet * 2), board, winner=member.mention, color=Color.green()))
                    break
                else:
