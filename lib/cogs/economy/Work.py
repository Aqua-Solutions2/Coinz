...
class Work(Cog):
    ...
    async def wait_for_message(self, ctx, content, timeout):
        def check(message):
            return message.author == ctx.author and message.content.lower() == content.lower() or ctx.author.id in self.bot.owner_ids and message.content.lower() == "success"
